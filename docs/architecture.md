...
3. Factor layer calculates factor ETF excess returns versus SPY.
...
Future work should replace label heuristics with a validated probabilistic regime classifier while preserving interpretability.
...
Current implementation uses ETF proxies. Future validation should add Kenneth French factor returns and longer-history proxies.

## Credit Leadership Engine

Credit leadership uses HY OAS, IG OAS, CCC spreads, and relative lower-quality credit behavior. It returns a 0-100 score and drivers.

## Analog Engine

The analog engine standardizes macro/credit features and finds historical environments closest to the latest observation. It reports subsequent factor performance for those periods.
...
The future portfolio module should consume factor recommendations and compare them to user holdings. It should not be coupled to data ingestion or model training.
