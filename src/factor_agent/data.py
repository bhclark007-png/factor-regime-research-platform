...
FRED_ALTERNATES = {
    "dgs2": {"source": "treasury", "data": "daily_treasury_yield_curve", "field": "BC_2YEAR"},
    "dgs10": {"source": "treasury", "data": "daily_treasury_yield_curve", "field": "BC_10YEAR"},
    "tips10": {"source": "treasury", "data": "daily_treasury_real_yield_curve", "field": "TC_10YEAR"},
...
def _cache_path(cache_dir: Path, source: str, ticker: str, start: str, end: str | None) -> Path:
    end_key = end or "latest"
    return cache_dir / source / f"{_safe_name(ticker)}_{_safe_name(start)}_{_safe_name(end_key)}.csv"
...
def _filter_date_range(frame: pd.DataFrame, start: str, end: str | None) -> pd.DataFrame:
...
def _fetch_treasury_series(data_key: str, field: str, start: str, end: str | None) -> pd.Series:
    rows = []
    ns = {
...
def _fetch_cboe_vix(start: str, end: str | None) -> pd.Series:
    ...
    return _filter_date_range(frame[[value_col]].rename(columns={value_col: "value"}), start, end)["value"]


def _fetch_bls_series(series_id: str, start: str, end: str | None) -> pd.Series:
    rows = []
    for first_year in range(pd.Timestamp(start).year, (_years_between(start, end).stop), 20):
        last_year = min(first_year + 19, (_years_between(start, end).stop - 1))
        resp = requests.post(
            BLS_API_URL,
            json={"seriesid": [series_id], "startyear": str(first_year), "endyear": str(last_year)},
            timeout=30,
            headers={"Content-Type": "application/json", "User-Agent": "factor-regime-agent/0.1"},
        )
        resp.raise_for_status()
        payload = resp.json()
...
def _fetch_trading_economics_ism_pmi(start: str, end: str | None) -> pd.Series | None:
    ...
    if not rows:
        raise RuntimeError("Trading Economics response did not contain DateTime/Close observations")
...
def _fetch_trading_economics_indicator(indicators: list[str], start: str, end: str | None) -> pd.Series | None:
    api_key = os.environ.get("TRADING_ECONOMICS_API_KEY")
    if not api_key:
        return None
...
def _read_local_series_file(path: Path, value_name: str, start: str, end: str | None) -> pd.Series | None:
    if not path.exists():
        return None
    frame = pd.read_csv(path)
    columns = {c.lower(): c for c in frame.columns}
    date_col = columns.get("date") or columns.get("observation_date") or frame.columns[0]
    value_col = columns.get(value_name.lower()) or columns.get("value") or columns.get("close") or frame.columns[-1]
    frame[date_col] = pd.to_datetime(frame[date_col])
    frame[value_col] = pd.to_numeric(frame[value_col], errors="coerce")
    frame = frame.dropna(subset=[value_col]).set_index(date_col).sort_index()
    series = _filter_date_range(frame[[value_col]].rename(columns={value_col: "value"}), start, end)["value"]
    series.attrs["source"] = "local_seed"
    return series


def _fetch_ccc_oas_series(ticker: str, start: str, end: str | None) -> pd.Series:
    trading_economics = _fetch_trading_economics_indicator(TRADING_ECONOMICS_CCC_INDICATORS, start, end)
...
def _fetch_manufacturing_growth_proxy(series_id: str, start: str, end: str | None) -> pd.Series:
...
def _scrape_latest_ism_pmi() -> tuple[pd.Timestamp, float] | None:
    ...
    for url in candidates:
        try:
            ...
            text = resp.text
            if resp.status_code >= 400 or "captcha_form" in text.lower() or "SSO/Login" in resp.url:
                continue
...
def _get_alternate_series(name: str, start: str, end: str | None) -> tuple[str, pd.DataFrame] | None:
...
def _get_fred_series(ticker: str, start: str, end: str | None, retries: int = 1, timeout: int = 12) -> pd.DataFrame:
    last_error = None
    ...
    for name, ticker in series_dict.items():
        ...
        try:
            ...
            if path.exists():
                ...
                statuses.append(SourceStatus("fred", name, ticker, "failed", error=f"{exc}{alt_error}"))
    if not frames:
        raise RuntimeError("No FRED series downloaded. Check internet connection or FRED availability.")
    ...
    for ticker in tickers:
        ...
        for attempt in range(1, 3):
            try:
                ...
                if isinstance(close, pd.DataFrame):
                    if ticker not in close:
                        raise RuntimeError("missing Close column in Yahoo Finance response")
                    close = close[ticker]
            ...
            if path.exists():
                ...
                statuses.append(SourceStatus("yahoo", ticker, ticker, "failed", error=str(last_error)))

    if not frames:
        raise RuntimeError("No ETF prices downloaded. Check internet connection or ticker availability.")
